...
public class TestPruebaOrdenados {
	...
	do {
		...
		case "Buscar":
			if(obj.validaVacio())
				...
				pos = obj.busquedaOrdenada(arreglo,tools.LeerEntero("Ingrese el valor"));
				byte p=obj.buscarSecuencial(tools.LeerEntero("Numero a buscar"));
				if(pos!=-2)
					tools.ImprimeMsje("Se encuentra en la posicion " + p);
		...
		case "Eliminar":
			if(obj.validaVacio()) tools.ImprimeErrorMsje("Array vacio");
				...
				byte index = obj.buscarSecuencial(tools.LeerEntero("Numero a buscar"));
				if (index != -1) {
